  ...
  describe "名刺作成機能" do
    context "ログインしている場合" do
      ...
      it "無効な情報では名刺を作成できず、エラーメッセージが表示されること" do
        visit new_business_card_path
        fill_in "名前", with: ""  # 名前は必須
        fill_in "会社名", with: "テスト株式会社"
        click_button "名刺を作成"
        expect(page).to have_content("入力エラーが発生しました:")
        expect(page).to have_content("名前を入力してください")
        expect(page).to have_current_path(business_cards_path)  # 再表示される
  ...
  describe "名刺編集機能" do
    context "ログインしている場合" do
      ...
      it "編集時にバリデーションエラーが発生した場合、エラーメッセージが表示されること" do
        ...
        visit edit_business_card_path(business_card)
        fill_in "名前", with: ""  # 名前は必須
        fill_in "会社名", with: "サンプル株式会社"  # 会社名は必須
        click_button "名刺を更新"
        expect(page).to have_content("入力エラーが発生しました:")
        expect(page).to have_content("名前を入力してください")
        expect(page).to have_current_path(business_card_path(business_card))  # 再表示される
        expect(page).to have_field("会社名", with: "サンプル株式会社")  # 入力値が保持されている
      end
    end
