...
%o`, message, error2, detail);
  ...
  var ApiClient = class {
    ...
    async request(endpoint, options = {}) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(error2.error || "API request failed");
        }
    ...
    async getBusinessCard(id) {
      const response = await this.request(`/business_cards/${id}`);
      return response.data;
    }
    // 名刺作成API - FormData対応
    // POSTリクエストで新規名刺を作成
    async createBusinessCard(data) {
      ...
      const response = await this.request("/business_cards", {
        method: "POST",
        body: formData,
        headers
      });
    ...
    async deleteBusinessCard(id) {
      await this.request(`/business_cards/${id}`, {
        method: "DELETE"
      });
    }
  };
  ...
  // app/javascript/utils/validation.ts
  ...
  function validateBusinessCardForm(formData) {
    const errors = {};
    ...
    if (formData.email !== void 0 && formData.email !== null) {
      const emailValidation = validateEmail(formData.email);
      if (!emailValidation.isValid) {
        errors.email = emailValidation.message;
      }
    }
    if (formData.phone !== void 0 && formData.phone !== null) {
      const phoneValidation = validatePhone(formData.phone);
      if (!phoneValidation.isValid) {
        errors.phone = phoneValidation.message;
      }
    }
    if (formData.website !== void 0 && formData.website !== null) {
